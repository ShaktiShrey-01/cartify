...
const Grocery = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        // Fetch only grocery products from backend
        const res = await fetch(`${API_BASE}/api/v1/products/getallproducts?categoryKey=grocery`);
        if (!res.ok) throw new Error(`Failed to load products: ${res.status}`);
        const json = await res.json();
        const groceryProducts = unwrapApiResponse(json);
        setProducts(Array.isArray(groceryProducts) ? groceryProducts : []);
      } catch (err) {
        console.error(err);
        setError('Could not load grocery products');
      } finally {
        setLoading(false);
      }
    };
