...
const Electronics = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        // Fetch only electronics products from backend
        const res = await fetch(`${API_BASE}/api/v1/products/getallproducts?categoryKey=electronics`);
        if (!res.ok) throw new Error(`Failed to load products: ${res.status}`);
        const json = await res.json();
        const electronicsProducts = unwrapApiResponse(json);
        setProducts(Array.isArray(electronicsProducts) ? electronicsProducts : []);
      } catch (err) {
        console.error(err);
        setError('Could not load electronics products');
      } finally {
        setLoading(false);
      }
    };
